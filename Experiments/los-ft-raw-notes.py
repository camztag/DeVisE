##################################################################################################
# Prompts for Length of Stay task on raw unstructured notes for fine-tuned models. 
# To be used with models with a Llama 3, Phi4, or DeepSeek R1 base, fine-tuned with these prompts. 
# Obtains the probability of the first 10 higher probs tokens (including the LOS bucket)
...
def format_prompt(template_name: str, note: str) -> str:
    instruction = (
        "You are an expert in hospital stay predictions. Predict the patient's total length of ICU stay "
        "based on their admission note summary. Output only a number between double brackets: [[1]] for under 3 days, "
        "[[2]] for 3-7 days, [[3]] for 1-2 weeks, [[4]] for more than 2 weeks."
    )
    input_text = f"Note:\n{note}"
...
def predict_bucket_probs(llm, data, output_path, batch_size=32, template_name="llama3"):
    results = []
    ...
    for i in tqdm(range(0, len(data), batch_size)):
        ...
        prompts = [format_prompt(template_name, item["text"]) for item in batch]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--template", type=str, choices=["llama3", "phi4", "deepseekr1"], default="llama3")
    ...
    print(f"Seconds per note: {(end_time - start_time) / len(sliced_data):.4f}")
